...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=19, patch_size=19, stride=2, in_chans=3, embed_dim=128, norm_layer=None,
                 flatten=True):
        super().__init__()
...
class VisionMamba(nn.Module):
    def __init__(self,
                 img_size=10,  ##daxiao xuyao gai
                 patch_size=1,
                 stride=2,
                 depth=24,
                 embed_dim=1, ##dim xuyaogai
                 channels=47,
                 num_classes=6400,
                 ssm_cfg=None,
                 drop_rate=0.,
...
class VisionMamba1(nn.Module):
    def __init__(self,
                 img_size= 19,  ##daxiao xuyao gai ABU：10/19 HAD：8
                 patch_size= 19,
                 stride=2,
                 depth=24,
                 embed_dim=1,  ##dim xuyaogai
                 channels=188,
                 num_classes=9400,  ### ABU 9400 HAD 6016
                 ssm_cfg=None,
                 drop_rate=0.,
                 drop_path_rate=0.1,
                 norm_epsilon: float = 1e-5,
...
class Encoder(nn.Module):
    ...
    def forward(self, x): #361 100 188
        ...
        x = x.reshape(x.shape[1], x.shape[2], x_sqrt, x_sqrt)
        ...
        # enc1 = self.act_f_1(self.BatchNorm1d_enc_layer3(self.encoder_layer3(e2)))
        # enc2 = enc1.reshape(x_sqrt, x_sqrt, enc1.shape[1])
        e1 = self.m1(x)
        print(e1.shape)
        dim0 = e1.shape[0]  # 获取第0维的大小
        dim0_s = torch.sqrt(torch.tensor(dim0))
        # 假设 x_sqrt 是一个标量，确保它是一个整数
        x_s = int(dim0_s.item())  # 获取 x_sqrt 的数值
        e1 = e1.reshape(e1.shape[0],-1, x_s, x_s)
        e2 = self.m2(e1)
        e2 = e2.reshape(e2.shape[0], -1, x_s, x_s)
        enc = self.m3(e2)
        print("Env",enc.shape)

        e1 = e1.reshape(dim0, e1.shape[0], -1)
        e2 = e2.reshape(dim0, e2.shape[0], -1)
        enc = enc.reshape(dim0, enc.shape[0], -1)
        print(enc.shape)
        return e1, e2, enc
...
class OrthoAE_Unet(nn.Module):
    ...
    def forward(self, x, args):
        ...
        return attention_fea, dec, dec, dec
