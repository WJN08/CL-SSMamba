...
class mambaLG(nn.Module):
    ...
    def forward(self, x, test=False):
        ...
        x = pad(x)
        bb, cc, hh, ww = x.shape
        print("bbcchh", x.shape)  ## 100 47 16 16
        ######spe########
        x_s = x
        ...
        x_s = Patch_pool(x_s)  # 100 47 1 1
        print("xsxsxs", x_s.shape)
        x_s = split_band(x_s, self.move_num, self.spec_num)
        b1, n1, c1, h1, w1 = x_s.shape
        x_s = rearrange(x_s, 'b n c h w-> (b h w) n c')
        x_s = self.SPEM(x_s)
        x_s = rearrange(x_s, '(b h w) n c-> b (n c) (h w)', h=h1, w=w1).mean(-1)
        x_s = self.nn2(x_s).unsqueeze(1).unsqueeze(1)
        print("xsxs", x_s.shape)  #100 1 1 47
        x_s = x_s.view(x_s.shape[0], x_s.shape[3], x_s.shape[1], x_s.shape[2])
        x = x_s * x
...
class Encoder(nn.Module):
    ...
    def forward(self, x): #361 100 188
        ...
        # e22 = self.m22(e11)
        # e22 = e22.reshape(e22.shape[0], -1, x_s, x_s)
        # enc1 = self.m33(e22)
        # enc1 = enc1.reshape(enc1.shape[0], -1, x_s, x_s)
        # print("e1", e11.shape)
        # print("e1", e22.shape)
        # print("Env", enc1.shape)
        #######################################################
