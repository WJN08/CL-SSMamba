...
def HAD(args):
    ...
    # AE.load_state_dict(torch.load("models/AE_0049.pth", map_location=lambda storage, loc: storage))

    enc_fea, Pretrain_DFAN, args = train_main1.pre_DFAN(data, dat, DFAN, AE, args)
    ...
    AD_result, _ = model_AE.RX(rec_result - date)
    ...
    AD_result -= np.min(AD_result)
    AD_result /= np.max(AD_result)
    ...
    plt.axis('off')
    plt.margins(0, 0)
    plt.savefig("Result/AU4.png", bbox_inches='tight', pad_inches=0.0)
    plt.show()

if __name__ == '__main__':

    parser = argparse.ArgumentParser()
    parser.add_argument('--data_dir', type=str, default="./Data/AU4_split.mat")
    # parser.add_argument('--data_dir', type=str, default="./Data/abu-urban-4.mat")
    # parser.add_argument('--data_dir', type=str, default="./Data/HAD100_100.mat")
    ...
    # # HSI_3D = np.array(dataset['data'], dtype=float)
    # # GT = np.array(dataset['map'], dtype=float)
    # # GT = sio.loadmat("Data/ang20170821t183707_91.mat")["gt"]
    # # GT = sio.loadmat("Data/ang20170821t183707_100.mat")["gt"]
    # # GT = sio.loadmat("Data/ang20191004t185054_24.mat")["gt"]
    # # GT = sio.loadmat("Data/ang20210614t141018_33.mat")["gt"]
